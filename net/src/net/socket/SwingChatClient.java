...
public class SwingChatClient extends JFrame implements ActionListener, Runnable{
	...
	private void connectPane() {
		...
		//127.0.0.1 = localhost 로  자기아이피 주소
		panel1.add(lb1);
		
		JLabel lb2 = new JLabel("대화명 : ");
		txtName = new JTextField("ㅎㅅㅎ",15);
		//127.0.0.1 = localhost 로  자기아이피 주소
		panel2.add(lb2);
	...
	public void chatPane(){
		...
		txtInput = new JTextField("",20); //텍스트필드를 그냥 비워두고 20글자 입력칸만 생성이란 의미
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == btnExit){
			System.exit(0); // 0이 왠진모르지만 꼭 있어야댐.
	...
	private void send() {
		...
		txtList.append(text+"\n");
		txtInput.setText(""); //인풋창의 글자를 지우고 초기화해라.
		txtInput.requestFocus(); // 입력창에 글자가 없으니까 커서만 깜빡깜빡하는 상태.
		pw.println(text); //pw는 printwriter 의 객체로 화면에 글자를 뿌려랑
		pw.flush(); // 물내림이란 뜻으로 pw 에게 가지고 있는 데이터를 전부
//						삭제하라는 뜻.
